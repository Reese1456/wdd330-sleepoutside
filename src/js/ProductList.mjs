...
export default class ProductList {
  ...
  async init() {
    ...
    const titleEl = document.querySelector(".products__title");
    if (titleEl) {
      const display = this.category.charAt(0).toUpperCase() + this.category.slice(1).replace(/-/g, " ");
      titleEl.textContent = `Top Products: ${display}`;
    }
    this.renderList(list);
  }

  renderList(list) {
    renderListWithTemplate(this.productCardTemplate.bind(this), this.listElement, list, "afterbegin", true);
  }
}
